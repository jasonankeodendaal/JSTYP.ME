...
const AdminDashboard: React.FC = () => {
    ...
    const renderContent = () => {
        switch (activeSection) {
            case 'brands': 
                if (!(loggedInUser?.isMainAdmin || perms?.canManageBrandsAndProducts)) return null;
                return <AdminBrandContent canManageBrands={!!(loggedInUser?.isMainAdmin || perms?.canManageBrandsAndProducts)} activeBulkImportTab={activeBulkImportTab} setActiveBulkImportTab={setActiveBulkImportTab} />;
            ...
            case 'content': 
                ...
                return (
                    <div>
                        ...
                        {activeContentTab === 'catalogues' && (loggedInUser?.isMainAdmin || perms?.canManageCatalogues) && <AdminCatalogueContent />}
                        {activeContentTab === 'pamphlets' && (loggedInUser?.isMainAdmin || perms?.canManagePamphlets) && <AdminPamphletContent />}
                    </div>
                );
            ...
            case 'kiosk':
                 ...
                 return (
                    <div>
                        ...
                        {activeKioskTab === 'screensaver' && (loggedInUser?.isMainAdmin || perms?.canManageScreensaver) && <AdminScreensaverAds />}
                        {activeKioskTab === 'tv' && (loggedInUser?.isMainAdmin || perms?.canManageTvContent) && <AdminTvContent />}
                    </div>
                );
